...
To prevent this, it would be possible to pass in a function to a Sandie instance _before_ loading a malicious script. Such a function could, for example, replace certain properties of the local @window@ object with @null@ values. The consequences of this might be that innocent uses for these properties would be made impossible. Different applications may want to lock down different properties.
